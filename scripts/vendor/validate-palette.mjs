 ...
 *
 * Copied verbatim from the `dataviz` skill's `scripts/validate_palette.js`.
 * Vendored so `pnpm check:palette` and `tests/chart-palette.test.ts` can run in
 * CI without the skill present. Re-copy it wholesale to update; local edits
 * would silently weaken the gates the palette is held to.
 *
 * Consumed by:
 *   scripts/check-chart-palette.mjs   (pnpm check:palette)
 *   tests/chart-palette.test.ts
 */
/**
...
const CVD_TARGET = 8.0,
  CVD_FLOOR = 6.0; // OKLab ΔE×100, min(protan, deutan), adjacent pairs
...
const splitColors = (raw) =>
  (raw || '').split(',').map(stripWs).filter(Boolean);
...
if (
  ...
  if (badHex.length) {
    console.error(
      `invalid hex value(s): ${badHex.join(', ')} — expected #rrggbb`,
    );
    process.exit(2);
  }
...
if (typeof document !== 'undefined') {
  ...
  if (b?.dataset.palette) {
    ...
    const rawSurface =
      b.dataset.surface != null ? stripWs(b.dataset.surface) : '';
    const surface = rawSurface || DEFAULT_SURFACE[mode];
