...
tldr: "AWS Batch runs arbitrary Docker containers for jobs too big for Lambda, without needing a big-data framework. This cookbook works through the core patterns problem-first — job dependencies for fan-in/fan-out and more — with the technical detail appended."
images: ["/static/batch/headers/cookbook-light.png"]
layout: PostLayout
authors: ["default"]
featuredLinks:
  - title: Older video about AWS Batch
    url: https://youtu.be/H8bmHU_z8Ac
  - title: Spot instance on Batch
    url: https://www.youtube.com/watch?v=qPU9FzuAKIw&ab_channel=AmazonWebServices
---
...
<MapReduceViz caption="Map/Reduce as Batch jobs flowing through a queue — one PREPARE job, a MAP array job draining through a slot-limited compute environment (one map loses its spot instance and re-queues), then REDUCE (Interactive simulation)" />
