...
- [ ] **One task end-to-end** — trace a cross-repo change: root AGENTS.md
      ...
      routed into the repo's own AGENTS.md → two PRs. The draft is all
      mechanism, no demonstration. (Agents bring their own search tools — the
      workspace's contribution is the map and the verbs.)
...
<NoteBlock title="out of scope: code-intelligence" color="red">
Drop the code-intelligence / code-graph material entirely — it's out of scope
for this article. The embedded draft below still leans on it (the "verbs"
section headlines a code-intelligence command); in the rewrite, rebuild that
section around the verbs that stay — `docs`, `help`, `sync`, task/observability
CLIs — and don't reintroduce the graph in any diagram or walkthrough.
</NoteBlock>

## Structural moves

- [ ] Group the mechanisms: **see** (filesystem + permissions), **know**
      (AGENTS.md + progressive disclosure), **do** (the CLIs / verbs),
      **don't** (guardrails).
...
- [ ] **Two-part series?** Part 1 the pattern, part 2 equipping the agents —
      ...
      A companion provisioning piece is already scoped:
      [Provisioning the Dev Environment](/ideas/workspace-devenv) (devenv +
      editor integration + the bespoke-vs-vendored tooling axis).
- [x] **Two-ignore-files, animated** — done, and better than a static diagram:
      the `<FileTree>` prototype below wires the symlink and toggles git-view
      vs agent-view on one path. Also encodes the factual fix (git tracks
      neither the code nor the symlink) in the git-view caption.
...
**The trick, on disk** — the one representation the other three don't give:
the structure itself. Wire the `projects → ../company` symlink to mirror the
body into the brain, then flip git-view vs agent-view to see the same path
ignored by git yet traversable by the agent — the whole pattern in one widget:

<FileTree title="the trick, on disk" />
...
<Walkthrough
  ...
  nodes={[
    ...
    { id: 'help', label: 'help command', detail: 'uv run help — every verb', x: 140, y: 190, accent: 'yellow' },
    { id: 'repo', label: 'repo AGENTS.md', detail: 'projects/<repo>/', x: 460, y: 190, accent: 'pink' },
  ]}
  edges={[
    { from: 'agent', to: 'root' },
    { from: 'root', to: 'help' },
  ...
  steps={[
    ...
    { title: 'Route into the repo', caption: 'The task points into one repo. Its own AGENTS.md takes over: local conventions, build commands, gotchas — progressive disclosure, the map handing off to the repo that owns the detail.', focus: ['root', 'repo'], activeEdges: ['root->repo'] },
    { title: 'Map + verbs', caption: 'The same loop a new starter runs on day one — read the map, run help, go deeper only where the task points. The whole system, one screen.', focus: ['agent', 'root', 'help', 'repo'] },
  ]}
/>
      ...
      context: [
        {
          ...
          lines: [
            ...
            '  workspace      branches · queue · roles · workflows',
            '',
            'TASKS & OBSERVABILITY',
