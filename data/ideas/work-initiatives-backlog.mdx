...
### B1. ★ Moving heavy compute off the web tier: AWS Batch in production (2019–2025) — *greenfield → optimisation*

The production sequel to the 2022 talk and the published cookbook: adopting
AWS Batch in 2019, array-job fan-out ("chunking up rows", 2022), Step
Functions as orchestrator, trace-context propagation across the job graph
(2021), the "empty file returned → lower parallelism" gotcha, chunk tuning
("decrease chunk size to 2 million"), and 2025 right-sizing of compute
environments. Ends the arc the talk started.

### B2. ★ The memory & concurrency wars (2021–2024) — *war story*

OOM hotfixes → streaming to Parquet for reduced memory → capping parallelism
to dodge S3 exhaustion → a `Parallel.For` race condition removed → a
connection-pool deadlock from releasing connections early → mutexes on
contract loading → tuning job weights. A multi-year campaign against
resource exhaustion in a data-heavy pipeline, with commit receipts
throughout. Best pure war-story material in the estate.
...
### B5. ★ Modernising a 10-year-old .NET Framework monolith — live (2024–2025) — *migration, marquee*

EF6 → EF Core and net472 → net8 with **no big-bang**: cross-compiling the
data layer, running both ORMs side-by-side, an EF Core migration checker in
CI, and the tree still honestly split (73 net8 vs 36 net472 projects).
Includes the production war story (an async fetch silently misbehaving on
the new ORM in production) and the consolidation revert that taught pace.
The dual solution files are the artefact.
...
Three verified campaigns — the .NET web/batch unification first (Dec 2023),
then the JS estate twice (2023–2025) — filter-repo mechanics, cut-point
branches as permanent markers, and the reference-link recovery task all
live there.
